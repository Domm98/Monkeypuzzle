...
// Function to add text or pdf to the resource pane
function add_tab(load_id = null) {
    ...
    if (resource_type_txt.toLowerCase() === "pdf") {
        if (pdf_count === 0) {
            pdf_count = 1;
			add_resource_header();
            add_pdf_resource_body(tab_id);
            set_active_tab(tab_id + "_body");
            return tab_id;
        }
        else {
            alert("You can only create one PDF resource pane, please open a new PDF file from the toolbar. Or, if you have deleted a PDF pane, simply refresh the page to open a new one.");
        }
    }
}
